...
import type { AforoData, AforoDataUpdate, WorksheetWithCase } from '@/types';
...
export function AforoHistoryModal({ isOpen, onClose, caseData }: AforoHistoryModalProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const fetchedHistory: AforoDataUpdate[] = [];
      querySnapshot.forEach((doc) => {
        fetchedHistory.push(doc.data() as AforoDataUpdate);
      });
  ...
  const renderChange = (item: AforoDataUpdate) => {
    ...
    if (item.field === 'status_change') {
      ...
      switch(item.newValue as AforoData['revisorStatus']) {
        case 'Aprobado':
