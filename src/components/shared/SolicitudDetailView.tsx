...
interface SolicitudDetailViewProps {
  ...
  onBackToList?: () => void;
  children?: ReactNode;
  solicitud?: SolicitudRecord | SolicitudData; // Can accept both full records and context data
...
export default function SolicitudDetailView({ id, isInlineView, onBackToList, children, solicitud: initialSolicitud, initialData: initialPassedData }: SolicitudDetailViewProps) {
  ...
  const [initialData, setInitialData] = useState<InitialDataContext | null>(initialPassedData || null);
  const [isLoading, setIsLoading] = useState(!initialSolicitud && !id);
  ...
  useEffect(() => {
    const fetchSolicitudData = async () => {
        if (initialSolicitud) {
            ...
            const dataForInitial = initialSolicitud as SolicitudRecord;
            setInitialData(initialPassedData || {
                ne: dataForInitial.examNe,
                reference: dataForInitial.examReference || undefined,
                manager: dataForInitial.examManager,
                date: dataForInitial.examDate || new Date(),
                recipient: dataForInitial.examRecipient,
                isMemorandum: dataForInitial.isMemorandum
            });
            setIsLoading(false);
        } else if (id) { // This is for when an ID is passed from the DB view
            setIsLoading(true);
            ...
            for (const collectionName of collectionsToTry) {
                ...
                if (docSnap.exists()) {
                    ...
                    const examDate = data.examDate instanceof FirestoreTimestamp ? data.examDate.toDate() : undefined;
                    const savedAt = data.savedAt instanceof FirestoreTimestamp ? data.savedAt.toDate() : undefined;
                    const record = { ...data, solicitudId: docSnap.id, examDate, savedAt } as SolicitudRecord;
                    setSolicitud(record);
                    setInitialData({
                        ne: record.examNe,
                        reference: record.examReference || undefined,
                        manager: record.examManager,
                        date: record.examDate || new Date(),
                        recipient: record.examRecipient,
                        isMemorandum: record.isMemorandum
                    });
                    found = true;
                    break;
                }
            ...
            if (!found) setError("No se pudo encontrar la solicitud.");
            setIsLoading(false);

        } else if (contextSolicitudes) { // This is for live context (examinerPay)
            const foundSolicitud = contextSolicitudes[contextSolicitudes.length - 1]; // Assume latest
            setSolicitud(foundSolicitud || null);
            setInitialData(contextInitialData);
            setIsLoading(false);
            if (!foundSolicitud) setError("No se pudo encontrar la solicitud en el contexto actual.");
        } else {
            setError("No hay datos de solicitud para mostrar.");
            setIsLoading(false);
        }
    };

    fetchSolicitudData();
  }, [id, initialSolicitud, initialPassedData, contextSolicitudes, contextInitialData]);
  ...
  const getBancoDisplay = (s: SolicitudData | SolicitudRecord) => {
  ...
  const getMonedaCuentaDisplay = (s: SolicitudData | SolicitudRecord) => {
        ...
        <CardContent className="pt-4 print:p-0">
             ...
             <div className="no-print">
                <CardHeader className="p-0 mb-4">
                    <CardTitle className="text-xl md:text-2xl font-semibold text-foreground">Detalle de Solicitud de Pago</CardTitle>
                    <CardDescription className="text-muted-foreground">Mostrando información de la solicitud seleccionada.</CardDescription>
                </CardHeader>
             </div>
               <div className="mb-3 p-4 border rounded-md bg-secondary/5 print:border print:mb-2">
                  <div className="flex justify-between items-center">
                      <DetailItem label="ID de Solicitud" value={(solicitud as SolicitudRecord).solicitudId || solicitud.id} icon={Info} />
                       <div className='flex items-center gap-2'>
                          ...
                          {solicitud.soporte && (
                              <Badge className="bg-green-100 text-green-700 text-sm font-semibold px-3 py-1 rounded-md border border-gray-300 print:border-gray-400 hover:bg-green-600 hover:text-white print:text-xs">
                              PAGADA
                              </Badge>
                          )}
                      </div>
              ...
              <div className="mb-3 p-4 border rounded-md bg-secondary/30 print:border print:mb-2">
                  <h3 className="text-lg font-semibold mb-2 text-primary print:text-base print:mb-1">Solicitud de Cheque</h3>
              ...
              <div className="mb-3 p-4 border border-border rounded-md bg-secondary/30 print:border print:mb-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 items-start mb-3 print:grid-cols-2 print:gap-x-8 print:mb-2">
                  <div className="flex items-baseline py-1"><Banknote className="h-4 w-4 mr-1.5 text-primary shrink-0 print:h-3 print:w-3" /><p className="text-sm text-foreground break-words print:text-xs print:font-bold">{formatCurrency(solicitud.monto, solicitud.montoMoneda)}</p></div>
  ...
  // This is the main return that structures the entire component
  return (
